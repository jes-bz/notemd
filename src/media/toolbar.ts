...
function copyClipboard(getContent: () => string, label: string) {
  return async () => {
    try {
      ...
      vscode.postMessage({
        command: 'info',
        content: `Copy ${label} successfully!`,
      })
    } catch (error) {
      vscode.postMessage({
        command: 'error',
        content: `Copy ${label} failed! ${error.message}`,
      })
    }
  }
}
  ...
  { name: 'edit-mode', tipPosition: 'e' },
  {
    ...
    toolbar: [
      ...
      'preview',
      {
        name: 'copy-markdown',
        icon: t('copyMarkdown'),
        click: copyClipboard(() => vditor.getValue(), 'Markdown'),
      },
      {
        name: 'copy-html',
        icon: t('copyHtml'),
        click: copyClipboard(() => vditor.getHTML(), 'HTML'),
      },
      {
        name: 'reset-config',
        icon: t('resetConfig'),
